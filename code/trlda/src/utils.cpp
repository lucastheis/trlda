...
MatrixXd TRLDA::signum(const MatrixXd& matrix) {
	return (matrix.array() > 0.).cast<double>() - (matrix.array() < 0.).cast<double>();
}



double TRLDA::gamma(double x) {
	if (x <= 0.0)
...
ArrayXXd TRLDA::gamma(const ArrayXXd& arr) {
...
ArrayXXd TRLDA::lngamma(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
ArrayXXd TRLDA::digamma(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
double TRLDA::polygamma(int n, double x) {
	if(n < 1)
		return digamma(x);
...
ArrayXXd TRLDA::polygamma(int n, const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
ArrayXXd TRLDA::tanh(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
ArrayXXd TRLDA::cosh(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
ArrayXXd TRLDA::sinh(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
ArrayXXd TRLDA::sech(const ArrayXXd& arr) {
	ArrayXXd result(arr.rows(), arr.cols());
...
Array<double, 1, Dynamic> TRLDA::logSumExp(const ArrayXXd& array) {
...
Array<double, 1, Dynamic> TRLDA::logMeanExp(const ArrayXXd& array) {
	Array<double, 1, Dynamic> arrayMax = array.colwise().maxCoeff() - 1.;
...
int TRLDA::sampleHistogram(const ArrayXd& histogram) {
...
ArrayXXd TRLDA::sampleNormal(int m, int n) {
...
ArrayXXd TRLDA::sampleGamma(int m, int n, int k) {
...
ArrayXd TRLDA::sampleDirichlet(const ArrayXd& alpha) {
	static mt19937 gen(rand());
...
ArrayXXd TRLDA::sampleDirichlet(int m, int n, double alpha) {
	static mt19937 gen(rand());
 ...
 */
ArrayXXi TRLDA::samplePoisson(int m, int n, double lambda) {
 ...
 * Algorithm due to Knuth, 1969.
 */
ArrayXXi TRLDA::samplePoisson(const ArrayXXd& lambda) {
...
ArrayXXi TRLDA::sampleBinomial(int w, int h, int n, double p) {
...
ArrayXXi TRLDA::sampleBinomial(const ArrayXXi& n, const ArrayXXd& p) {
...
set<int> TRLDA::randomSelect(int k, int n) {
	if(k > n)
...
VectorXi TRLDA::argSort(const VectorXd& data) {
...
MatrixXd TRLDA::covariance(const MatrixXd& data) {
...
MatrixXd TRLDA::covariance(const MatrixXd& input, const MatrixXd& output) {
...
MatrixXd TRLDA::corrCoef(const MatrixXd& data) {
...
MatrixXd TRLDA::normalize(const MatrixXd& matrix) {
	return matrix.array().rowwise() / matrix.colwise().norm().eval().array();
}



MatrixXd TRLDA::pInverse(const MatrixXd& matrix) {
	if(matrix.size() == 0)
...
double TRLDA::logDetPD(const MatrixXd& matrix) {
	return 2. * matrix.llt().matrixLLT().diagonal().array().log().sum();
}



MatrixXd TRLDA::deleteRows(const MatrixXd& matrix, vector<int> indices) {
...
MatrixXd TRLDA::deleteCols(const MatrixXd& matrix, vector<int> indices) {
